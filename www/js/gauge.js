var gauge=function(container,configuration){
	...
	var config={
	size: 200,
	clipWidth: 200,clipHeight: 110,
	ringInset: 20,ringWidth: 20,
	...
	function configure(configuration){
		var prop=undefined;
		for(prop in configuration){config[prop]=configuration[prop];}
	...
	function isRendered() {return (svg !== undefined);}
	...
	function render(newValue) {
		...
		// Cambio scala
		var nticks=[];for(var i=0; i<ticks.length; i++) nticks.push(ticks[i]-100);
		lg.selectAll('text').data(nticks)
			.enter().append('text')
				.attr('transform',function(d){
					var ratio=scale(d+100); // Compensazione cambio scala
					var newAngle = config.minAngle + (ratio * range);
	...
	function update(newValue,newConfiguration){
		if(newConfiguration!==undefined){configure(newConfiguration);}
