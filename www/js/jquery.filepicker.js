+function ( $, window) {
  ...
  var _styles = {
    ...
    'jquery-ui-mobile': {
            ui: [
              { element: '#{ preview }', className: 'filepicker-preview'}, 
              { className: 'filepicker-ui', childNodes: [{ element: '#{ button }', className: 'ui-btn ui-shadow ui-corner-all', innerHTML: '<%= element.getAttribute("data-label") %>' }, { className: 'ui-input-text ui-body-inherit ui-corner-all ui-shadow-inset', childNodes:[{ element: '#{ input }'}] }]},  
              { element: '#{ element }' }
            ], 
            thumbnail: {
              wrap: '<div class="ui-body-inherit ui-corner-all ui-shadow-inset" style="display: none; text-align: center;"></div>', 
              style: "max-width: 100%"
            }
          }
  };
